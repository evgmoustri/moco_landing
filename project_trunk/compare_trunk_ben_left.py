...
results_path = os.path.abspath(project_path +
                               "/Results/Compare/predict_trunk_effort0.002_2/")
if not os.path.isdir(results_path):
    os.makedirs(results_path)
results_path_trunk = os.path.abspath(project_path +
                                     "/Results/predict_trunk_effort0.002_2")

# Internal
name = 'predict_trunk_ben_left'
scenarios = ['ext_0', 'ben_-5', 'ben_-10', 'ben_-15', 'ben_-20', 'ben_-25.1',
             'ben_-30']
...
fig1.suptitle('Drop - Landing: Lumbar Left Bending case study',
...
fig3.suptitle('Drop - Landing: Lumbar Left Bending case study',
              fontsize=26, fontweight='bold')
...
for idx, sen in enumerate(scenarios):
    ...
    #                  marker='x',markevery=marker_on)
    marker_on = [max_fy_id[idx]]
    line_2, = ax2.plot(time_arrays_2[:], savgol_filter(ham_force,9,3,
                      mode='nearest'),color=my_colors[idx],
                     marker='x',markevery=marker_on)
    marker_on = [max_fy_id[idx]]
    line_3, = ax3.plot(time_arrays_2[:], savgol_filter(qh_ratio,9,3,
                    mode='nearest'), color=my_colors[idx],
                     marker='x',markevery=marker_on)
    marker_on = [max_fy_id[idx]]
    line_4, = ax4.plot(time_arrays_2[:], savgol_filter(gastro_force,9,3,
                     mode='nearest'),  color=my_colors[idx],
                     marker='x',markevery=marker_on)
    marker_on = [max_fy_id[idx]]
    line_5, = ax5.plot(time_arrays_2[:],savgol_filter(ta_force,9,3,
                     mode='nearest'),  color=my_colors[idx],
                     marker='x',markevery=marker_on)
    marker_on = [max_fy_id[idx]]
    line_6, = ax6.plot(time_arrays_2[:], savgol_filter(gta_ratio,9,3,
                     mode='nearest'),  color=my_colors[idx],
                     marker='x',markevery=marker_on)
...
fig_sol.suptitle('Drop - Landing: Lumbar Left Bending case study',
              fontsize=26, fontweight='bold')
