...
at_dict = {
    1: {'scenario': 'trunk_all_free', 'e': 1, 'b': 1, 'r': 1},
    2: {'scenario': 'trunk_ext_free', 'e': 1, 'b': 0.01, 'r': 0.01},
    3: {'scenario': 'trunk_rot_free', 'e': 0.01, 'b': 0.01, 'r': 1},
    4: {'scenario': 'trunk_ben_free', 'e': 0.01, 'b': 1, 'r': 0.01},
    5: {'scenario': 'trunk_all_restr', 'e': 0.01, 'b': 0.001, 'r': 0.01},
}
...
for i in range(len(at_dict)):
    ...
    e = float(at_dict[i+1]['e'])
    b = float(at_dict[i+1]['b'])
    r = float(at_dict[i+1]['r'])

    results_path = os.path.abspath(project_path +
                                   "/Results/predict_trunk_free_ef_" + str(
                                               w_eff) + '/' + scenario)
    ...
    # Set Goals
    problem.addGoal(osim.MocoControlGoal('effort', w_eff))
