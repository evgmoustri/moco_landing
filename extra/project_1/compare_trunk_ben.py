...
my_colors = ['red', 'salmon', 'purple', 'orange', 'tan', 'black', 'gray',
             'goldenrod', 'yellow', 'olive', 'seagreen']

scenarios = ['ben_-20', 'ben_-15', 'ben_-10',
             'ben_-5', 'track', 'ben_5', 'ben_10', 'ben_15', 'ben_20']
scenarios_2 = ['ben_-20', 'ben_-15', 'ben_-10',
...
for sen in scenarios:
    ...
    # in_idx = np.where(time_array == 0.19)
    # in_idx =in_idx[0][0]
    # fin_idx = np.where(time_array == 0.295)
    # fin_idx = fin_idx[0][0]
...
for idx, fit in enumerate(scenarios):
    line, = ax1.plot(time_array, fx[:, idx],
                     color=my_colors[idx])
    handles.append(line)
...
plt.savefig(results_path + '/GRF_ben.png')
plt.show()
...
plt.savefig(results_path + '/JRA_ben.png')
plt.show()
print('ok')
