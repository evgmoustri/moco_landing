...
for i in range(len(at_dict)):
    ...
    results_path = os.path.abspath(project_path +
                                   "/Results/predict_flat-foot_effort0.001/" +
                                   scenario)
    ...
    problem.setStateInfo("/jointset/ground_pelvis/pelvis_ty/value",
                         ...
                         osim.MocoFinalBounds(0.94, 0.95))
    ...
    problem.setStateInfo("/jointset/hip_l/hip_flexion_l/value", osim.MocoBounds(
        0.08, np.deg2rad(5)), osim.MocoInitialBounds(
        0.087),osim.MocoFinalBounds(np.deg2rad(5)))
    problem.setStateInfo("/jointset/knee_l/knee_angle_l/value",
                         osim.MocoBounds(-0.2, -0.15),
                         osim.MocoInitialBounds(-0.2), osim.MocoFinalBounds(
            np.deg2rad(-10)))
    problem.setStateInfo("/jointset/ankle_l/ankle_angle_l/value",
                         osim.MocoBounds(-0.6, np.deg2rad(5)),
                         osim.MocoInitialBounds(-0.6),osim.MocoFinalBounds(
            np.deg2rad(5)))
